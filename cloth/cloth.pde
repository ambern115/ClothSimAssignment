...
static class ClothParams {
  ...
  static float restLen = 5; //the resting length of each springs
  ...
  static double airVelX =  32000;
  static double airVelY = - 2000;
  static double airVelZ =   4000;//55000;
  ...
  static int springSystemHeight = 50; //the number of nodes on the tall side of the spring system
  static int springSystemLength = 70; //the number of nodes on the long side of the spring system
...
void setup() {
  ...
  //print(ss.toString());
  trans = loadImage("transpride.png");
  murica = loadImage("murica.jpg");
...
void displayHUD() {
  ...
  text(("                 Number of Spring Nodes: " + ClothParams.springSystemHeight*ClothParams.springSystemLength +
          ...
          "\n                  Physics Updates per Second: " + ss.currPhysFramerate), 4, -18);
}
...
void draw() {
  ...
  addUserAirVel(); //adds user-determined air velocity to the system
  ...
  vertex(width*11, height*2, 1000, 14, 0);
  vertex(width*11, height*2, -100000, 14, 84);
  vertex(-width*10, height*2, -100000, 0, 84);
  endShape();
