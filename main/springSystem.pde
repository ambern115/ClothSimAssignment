class SpringSystem {
  ...
  void add_node() {
    ...
    Node n = new Node(45,h-50,h,0.5,radius,floor_height);
    nodes.add(n);
    num_nodes++;
  }
  ...
  void update() {
    ...
    for (int i = num_nodes-1; i > -1; i--) {
      if (i > 0) {
        ...
        nodes.get(i).update(nodes.get(i-1).prev_velY, num_nodes, total_forces_below);
        total_forces_below += nodes.get(i).total_forceY/nodes.get(i).mass;
      } else {
        nodes.get(i).update(0, num_nodes, total_forces_below);
        total_forces_below += nodes.get(i).total_forceY/nodes.get(i).mass;
      }
    } 
  }
